...
#wandb stuff
run = wandb.init(project="fsp", group="sshet_mhafsef",
                 config={
                    "batch_size" : 2 ** 12,
                    "gnn" : "hgt",
                    "lr" : 1e-5,
...
gnn_config["num_types"] = len(data.node_map)
...
def train(epoch):
    ...
    neg_train_edge_loader = DataLoader(torch.arange(train_neg_edge_index.size()[-1]),
                                   batch_size=wandb.config.batch_size, shuffle=True)
    zero_node_types = torch.zeros_like(node_types).to(device)
...
mha_early_stopping = experiment.get_early_stopping(patience=experiment.patience, delta=0.1, verbose=True, prefix=f"mha")
best_loss = np.inf

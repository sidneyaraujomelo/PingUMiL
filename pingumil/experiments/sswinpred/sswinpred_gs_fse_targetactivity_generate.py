...
while current_timestamp < max_timestamp:
    ...
    pd.DataFrame.from_dict(embedding_dict).to_csv("pingumil/experiments/sswinpred/embeddings/embedding_dict.csv")
    torch.save(torch.stack(player1_embs), 'pingumil/experiments/sswinpred/embeddings/player1embs.pt')
    torch.save(torch.stack(player2_embs), 'pingumil/experiments/sswinpred/embeddings/player2embs.pt')
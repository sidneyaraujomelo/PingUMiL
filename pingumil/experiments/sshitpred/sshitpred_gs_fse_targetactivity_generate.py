...
project_name = "hitpred_binary_activity"
group_name = "generate_embeddings_lowerlr"
wandb_config = {
    "batch_size" : 1024,
    "lr": 1e-4,
    "lr_clf" : 1e-4,
...
while current_timestamp < max_timestamp:
    ...
    for graph_id, graph_entry in dataset.items():
        embedding_dict["timestamp"].append(current_timestamp)
        embedding_dict["graph"].append(graph_id)
        ...
        player1_embs.append(z[target_p1_node_idx])
        ...
        player2_embs.append(z[target_p2_node_idx])
    ...
    pd.DataFrame.from_dict(embedding_dict).to_csv("pingumil/experiments/sshitpred/embeddings/embedding_dict.csv")
    torch.save(torch.stack(player1_embs), 'pingumil/experiments/sshitpred/embeddings/player1embs.pt')
    torch.save(torch.stack(player2_embs), 'pingumil/experiments/sshitpred/embeddings/player2embs.pt')
...
def train_test_routine(model_name, trainval_df, test_df, parameters, data_columns, experiment_control_dict,
    ...
    sgkf_train_val = StratifiedGroupKFold(n_splits=4, shuffle=True, random_state=43)
    best_param = None
    best_avg_f1 = 0
    for param in list(ParameterGrid(parameters)):
        datapoints_idx = [x for x in range(len(trainval_df)) if x%5==0]
        groups = trainval_df.loc[datapoints_idx, match_column].values
        classes = trainval_df.loc[datapoints_idx, class_column].values
        f1_val_toavg = []
        for fold, (train_index, val_index) in enumerate(sgkf_train_val.split(datapoints_idx, classes, groups)):
            if len(val_index) == 0:
                continue
            train_df = trainval_df.loc[train_index]
            val_df = trainval_df.loc[val_index]
            
            X_train, y_train = get_data(model_name, train_df, data_columns, match_column, class_column)
            X_val, y_val = get_data(model_name, val_df, data_columns, match_column, class_column)
    ...
    X_test, y_test = get_data(model_name, test_df, data_columns, match_column, class_column)
    y_test_hat = None
    X_trainval, y_trainval = get_data(model_name, trainval_df, data_columns, match_column, class_column)
    if model_name == "asm":
...
def get_data(model_name, df, data_columns, match_column, class_column):
    ts_index = [x for x in df.index if x%5==0]
    X = []
    y = []
    for i in range(len(ts_index)):
        new_x = None
        if model_name == "asm":
            new_x = df.loc[ts_index[i]:ts_index[i]+4, data_columns+[match_column, class_column]]
        else:
            new_x = df.loc[ts_index[i]:ts_index[i]+4, data_columns]
        if (len(new_x)) == 5:
            X.append(new_x)      
...
def run(model_name, add_cluster=False, num_clusters=5, base_clustering="kmeans", run_csf=True, parameters = None):
    ...
    experiment_control_dict, output_dict = train_test_routine(model_name, train_df, test_df,
...
if __name__ == "__main__":
    ...
    #for model_name in ["rf"]:
        for run_csf in [False, True]:
            if model_name == "asm" and run_csf:
                continue

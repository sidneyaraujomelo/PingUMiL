...
#wandb stuff
run = wandb.init(project="fsp", group="gpp_mhafsef",
                 config={
                    "batch_size" : 512,
                    "lr": 1e-3,
                    "lr_clf": 1e-5,
...
experiment = GPPBaseExperiment(experiment_tag="semisup_mhafsef",
                               epochs=1000, 
                               mlc_epochs=200,
                               timestamp=time.time(),
...
for graph_id, graph_data in dataset.items():
    ...
    cat_node_feats = torch.cat(node_feats)
    cat_node_typencs = torch.cat(node_typencs)
    dataset[graph_id]["node_feats"] = torch.cat((cat_node_feats, cat_node_typencs), dim=1)
...
n_head = 8
first_graph_data = list(dataset.values())[0]["graph"]
d_model = first_graph_data.x.shape[-1]
d_k = first_graph_data.x.shape[-1]
d_v = first_graph_data.x.shape[-1]
...
def train(epoch):
    ...
    for _, graph_data in dataset.items():
        ...
        data_to_transform = torch.utils.data.DataLoader(x,
...
gnn_early_stopping = experiment.get_early_stopping(verbose=True,
                                                   prefix="embed")
mha_early_stopping = experiment.get_early_stopping(verbose=True,
                                                   prefix=f"mha")
best_loss = np.inf

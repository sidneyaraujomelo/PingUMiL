...
from sklearn.model_selection import LeaveOneOut, StratifiedKFold, StratifiedGroupKFold, train_test_split
...
class SSHitPredBaseExperiment():
    def __init__(
        ...
        self.train_df = pd.read_csv(
            "/raid/home/smelo/PingUMiL-pytorch/dataset/SmokeSquadron/ss_hitprediction/hitprediction_dataset_5s_train.csv",
            index_col=False)
        self.test_df = pd.read_csv(
            "/raid/home/smelo/PingUMiL-pytorch/dataset/SmokeSquadron/ss_hitprediction/hitprediction_dataset_5s_test.csv",
            index_col=False)
    ...
    def split_train_df(self, n_splits):
        sgkf_train_val = StratifiedGroupKFold(n_splits=n_splits, shuffle=True, random_state=42)
        return sgkf_train_val.split(self.train_df.index, self.train_df["class"], self.train_df["source"])
          
    def classification_step(self, device, model, X_trainval, y_trainval, groups_trainval,
                            X_test, y_test, groups_test, wandb):
        self.log(f"Classification Model: {model}")
        experiment_control_dict = self.create_output_dict()
        output_dict = self.create_output_dict()
        
        sgkf_train_val = StratifiedGroupKFold(n_splits=9, shuffle=True, random_state=42)
        
        for fold, (train_index, val_index) in enumerate(sgkf_train_val.split(X_trainval, y_trainval, groups_trainval)):
        ...
        self.save_experiment_dicts(experiment_control_dict, output_dict)
        self.finish()
            
    def save_experiment_dicts(self, experimental_control_dict, output_dict):
        experiment_df = pd.DataFrame.from_dict(experimental_control_dict)
        experiment_df.to_csv(f"{self.log_folder}/hp_pingumil_trainval.csv")
        print(experiment_df.describe())

        output_df = pd.DataFrame.from_dict(output_dict)
        output_df.to_csv(f"{self.log_folder}/hp_pingumil_test.csv")

...
model_path = "/raid/home/smelo/PingUMiL-pytorch/experiment_log/winpred_embedding_generation/upbeat-sun-3/"
typeproj_model = torch.load(os.path.join(model_path, "proj_1723755927.1221068.pt"))
sage_model = torch.load(os.path.join(model_path, "embed_1723755927.1221068.pt"))

classification_config = json.load(open(experiment.model_config))[1]
classification_config["in_channels"] = sage_model["convs"][-1].out_channels
...
def learning_routine(epoch, data_dict, sage_model, typeproj_model, clf_model,
    ...
    for graph_id, graph_data in data_dict.items():
        ...
        loss = F.binary_cross_entropy_with_logits(y_hat, y_train)
...
def test_routine(data_dict,
    ...
    for graph_name, graph_data in data_dict.items():
        ...
        np1_y_pred_test = torch.round(torch.sigmoid(player01_y_hat)).cpu().detach().numpy()
        np2_y_test = player02_y_test.cpu().detach().numpy()
        np2_y_pred_test = torch.round(torch.sigmoid(player02_y_hat)).cpu().detach().numpy()
        ...
        pred_p1_dict = experiment.create_prediction_dict(data.player01_idxs, [graph_name]*X_z_p1.size()[0], np1_y_test, np1_y_pred_test)
        pred_p2_dict = experiment.create_prediction_dict(data.player02_idxs, [graph_name]*X_z_p2.size()[0], np2_y_test, np2_y_pred_test)
...
for epoch in range(1, experiment.epochs):
    ...
    if clf_early_stopping.early_stop:

...
def run(add_cluster=False, num_clusters=5, base_clustering="kmeans", output_base_filename = "rf_sk10f"):
    ...
    for timestamp in unique_timestamps:
        ...
        if y.count('player01') <= 10 or y.count('player02') <= 10:
                continue
        train_val_index, test_index = next(kf_train_val.split(X, y))
...
if __name__ == "__main__":
    run()
    run(add_cluster=True, num_clusters=5, base_clustering="kmeans", output_base_filename = "rf_sk10f")
    for i in [5, 10, 20]:
        run(add_cluster=True, num_clusters=i, base_clustering="spectralg55", output_base_filename = "rf_sk10f")
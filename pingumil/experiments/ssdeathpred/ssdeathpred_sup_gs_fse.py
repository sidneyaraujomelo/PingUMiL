...
def update_dataset_with_cls_data(df, dataset):
    for _, row in df.iterrows():
        ...
        dataset[graph_id]["graph"].cls_x.append((source_id, target_id))
        dataset[graph_id]["graph"].y.append(row["class"])
...
classification_config["in_channels"] = sage_config["out_channels"]*2
...
def learning_routine(epoch, data_dict, sage_model, typeproj_model, clf_model,
    ...
    total_loss = 0
    total_acc = 0
    for _, graph_data in data_dict.items():
        data = graph_data["graph"]
        x = data.x.to(device)
        if stage == "train":
            sage_model.train()
        ...
        X_z = []
        for cls_x in data.cls_x:
            player_node_zs = [z[x,:] for x in cls_x]
            X_z.append(torch.cat(player_node_zs))
        X_z = torch.stack(X_z)
...
def test_routine(data_dict,
    ...
    test_f1 = []
    test_acc = []
    X_test_idx, groups_test, y_test, y_test_hat = []
    i=0
    for _, graph_data in data_dict.items():
        data = graph_data["graph"]
        x = data.x.to(device)
        ...
        acc, p, r, f1 = experiment.calculate_metrics(np_y_train, np_y_pred_train)
        wandb.log({f"graph test acc": acc})
        wandb.log({f"graph test p": p})
        wandb.log({f"graph test r": r})
        wandb.log({f"graph test f1": f1})
    ...
    return (test_acc, test_p, test_r, test_f1), i
...
train_index, val_index = next(experiment.split_train_df(9))
...
test_routine(test_data_dict, sage_model, typeproj_model, classification_model)
